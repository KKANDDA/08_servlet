...
@WebServlet(value = "/annotation-lifecycle", loadOnStartup = 2) // 웹앱에 인덱스에서 찾을 수 있다.
public class LifeCycleTestServlet extends HttpServlet {

    // annotataion 방식
    // - 현대적 방식으로 서블릿 클래스 위에 @WebServlet 어노테이션을 사용하여 직접 매핑하는 방식
    ...
    public void init(ServletConfig config) throws ServletException {
        System.out.println("annotation 매핑 init()호출: " + initCount++); // 사용자가 들어오기 전 준비하는 과정
    }

    @Override
    public void service(ServletRequest req, ServletResponse res) throws ServletException, IOException {
        System.out.println("annotation 매핑 service() 호출: " + serviceCount++);
        // 최초 요청 시에는 init() 이후에 동작하고, 두 번째 요청 부터는 init()호출 없이 바로 호출된다.
    }
    /*
