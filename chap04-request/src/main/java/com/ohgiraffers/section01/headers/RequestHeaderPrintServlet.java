...
public class RequestHeaderPrintServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        * 1. General header: 요청 및 응답 모두 적용되지만 body와는 관련 없는 헤더
        * 2. Request header: fetch될 리소스나 클라이언트 자체에 대한 상세 정보를 포함하는 헤더
        * 3. Respinse header: 위치나 서버 자체와 같은 응답에 대한 부가적인 정보를 갖는 헤더
        * 4. Entity header: 컨텐츠 길이나 엔티티 body에 대한 상게 정보를 포함하는 헤더
        *
        * 출력시 나오는 헤더는 요청헤더(Request header)이다.
        * */

        Enumeration<String> headerNames = req.getHeaderNames(); // 열거타입
        while (headerNames.hasMoreElements()) {
            System.out.println(headerNames.nextElement()); // 리퀘스트 헤더에 담긴
        }
        ...
        // host: 서버의 도메인 네임과 현재 Listhning 중인 TCP 포트 지정
