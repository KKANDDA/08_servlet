...
public class ContextListenerTest implements ServletContextListener, ServletContextAttributeListener {
    ...
    1. context(톰캣 컨테이너 자체에 리스너 연결)
        ...
        1-2: ServletContextAttributeListener: ServletContext에 속성을 추가, 제거, 수정하는 것에 대한 이벤트 리스너
    * */

    @Override
    public void contextInitialized(ServletContextEvent sce) {
        // context가 생성될 때 변화를 감지하는 Listener 인스턴스가 함께 생성된다.
        // 최초 생성시 생성자 호출 이후 동작 메소드
    ...
    public void contextDestroyed(ServletContextEvent sce) {
        // context가 소멸할 때 동작한다.
        System.out.println("context listener 소멸");
    }

    @Override
    public void attributeAdded(ServletContextAttributeEvent event) {
        System.out.println("context Attribute added!!");
        // context의 attribute가 추가될 때 동작한다.
    }
